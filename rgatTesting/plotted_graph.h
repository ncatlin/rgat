...
{
public:
	plotted_graph(PROCESS_DATA *processdata, unsigned int threadID);
	~plotted_graph();
...
/*
	...
	virtual void render_edge(NODEPAIR ePair, GRAPH_DISPLAY_DATA *edgedata, map<int, ALLEGRO_COLOR> *lineColours,
		ALLEGRO_COLOR *forceColour, bool preview, bool noUpdate);

protected:
	virtual void positionVert(int *pa, int *pb, int *pbMod, MEM_ADDRESS address, char lastInstructionType);
	...
	std::multimap<pair<int, int>, bool> usedCoords;	

	//get on-screen xyz coords of node
	virtual FCOORD nodeCoordB(MULTIPLIERS *dimensions, float diamModifier, unsigned int index);
	virtual void drawHighlight(unsigned int nodeIndex, MULTIPLIERS *scale, ALLEGRO_COLOR *colour, int lengthModifier);
	virtual int add_node(node_data *n, GRAPH_DISPLAY_DATA *vertdata, GRAPH_DISPLAY_DATA *animvertdata,
	...
	virtual void draw_edge_heat_text(VISSTATE *clientState, int zdist, PROJECTDATA *pd);
	virtual void draw_condition_ins_text(VISSTATE *clientState, int zdist, PROJECTDATA *pd, GRAPH_DISPLAY_DATA *vertsdata);

	unsigned long lastVertID;
