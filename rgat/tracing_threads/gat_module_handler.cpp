...
void gat_module_handler::main_loop()
{
	...
	if (!inputPipe)
	{
		bool pipeConnected = establishPipe();
		if (!pipeConnected)
			return;
	}
	...
	while (true)
	{
		bytesRead = getData(buf, ov2, pendingControlCommand);
		buf[bytesRead] = 0;
...
bool gat_module_handler::establishPipe()
{
	...
	if (die) { alive = false;  return false; }
...
DWORD gat_module_handler::getData(char *buf, OVERLAPPED ov2, bool &pendingControlCommand)
{
	DWORD bytesRead = 0;
...
void gat_module_handler::handlePipeError()
{
...
void gat_module_handler::handleSymbol(char *buf)
{
...
void gat_module_handler::handleNewThread(char *buf, OVERLAPPED &ov2)
{
	PID_TID TID = 0;
	...
	switch (runRecord->getTraceType())
	{
	case eTracePurpose::eVisualiser:
		handleNewVisualiserThread(TID, ov2);
		break;
	...
	default:
		wcerr << "[rgat] Bad Trace Type " << runRecord->getTraceType() << std::endl;
		break;
	}
}

void gat_module_handler::handleNewVisualiserThread(PID_TID TID, OVERLAPPED &ov2)
{
...
void gat_module_handler::end_threads()
{
